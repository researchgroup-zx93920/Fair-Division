...
// ----------------------- Description V4: ---------------------
// Primarily for testing - see if min sum dX alone will be able to generate same output or not, checking specific samples
// X (allocation) = binary, Y (fPO) = binary, Objective = min (sum dX + sum dY), 
...
int main() {
    ...
    int numberOfPoolSolutions = 10;        // number of Solutions to find for the main LP
    ...
    while(iteration < samples) {
        ...
        // ---------------------------------- GUROBI CODE -------------------------------------
        try {
            // setup gurobi environment and define model 
            ...
            // add X - binary allocation variable
            ...
            // add Z variable - alias for maximum disutility chore
            vector<GRBVar> Z(n);
            for(int i = 0; i < n; i++) {
            ...
            // add auxillary variables for multiplying coefficients (since GenConstrMax cannot take c*X type variables)
            GRBVar A[n][m];
            for(int i = 0; i < n; i++) {
                for(int k = 0; k < m; k++) {
            ...
            EF1_fPO_model.set(GRB_IntParam_PoolSolutions, numberOfPoolSolutions);
            ...
            if (optimstatus == GRB_OPTIMAL) {
                ...
                for (int e = 0; e < nSolutions; e++) {
                    ...
                    // ------------------------------------------------------------------------------------------------------------
                    // ------------------ check for fPO and report difference between objectives - Method 1 -----------------------
                    // double X_sum = 0;
                    // double Y_sum = 0;
                    // for(int i = 0; i < n; i++) {
                    //     for(int k = 0; k < m; k++) {
                    ...
                    //     }
                    // }
                    // Y_sum = EF1_fPO_model.get(GRB_DoubleAttr_PoolObjVal) - X_sum;
                    ...
                    // // if the X sum is equal to Y sum, X is fPO, else it is not
                    // if(doubleIsEqual(X_sum, Y_sum, EPS)==true) {
                    ...
                    // cout << "Y objective: " << (EF1_fPO_model.get(GRB_DoubleAttr_PoolObjVal) - X_sum) << "\n\n";
                    // continue; // skip method 2

                    // ------------------------------------------------------------------------------------------------------------
                    // ------------------ check for fPO and report difference between objectives - Method 2 -----------------------
                    GRBEnv * env1 = 0;
                    env1 = new GRBEnv ();
                    ...
                    if (optimstatus_fPO == GRB_OPTIMAL) {
                        ...
                        for(int i = 0; i < n; i++) {
                            for(int k = 0; k < m; k++) {
                                X_sum+=(agents[i].itemUtilityMap[k]*(X[i][k].get(GRB_DoubleAttr_Xn)));
                                Y_sum+=(agents[i].itemUtilityMap[k]*(Y[i][k].get(GRB_DoubleAttr_Xn)));
