...
vector<double> findEFMaxBundlePrice(vector<AgentNodes> agents, vector<ItemNodes> items) {
    double EFMaxBundlePrice = 0;
    int temp = 0;
    // cout << "Possible Big Spender -> ";
    for(int i = 0; i < agents.size(); i++) {
        ...
        if(doubleIsEqual(EFMaxBundlePrice, (agents[i].bundlePrice - maxItemPrice), EPS)==true) {
            temp = i;
            // cout << temp << " ";
        }
    }
    // cout << endl;
    // cout << "Allocation items-> ";
    // for(auto g:agents[temp].allocationItems)
        // cout << g->index << " ";
    // cout << endl;
    return {EFMaxBundlePrice, (double) temp};
}
...
double computeAlpha3(unordered_set<int> LSComponentAgents,  vector<AgentNodes> agents, double EFMaxBundlePrice) {
    double alpha3 = numeric_limits<double>::max();
    for(auto LSAgent:LSComponentAgents) {
        double maxItemPrice = 0;
        for(auto item:agents[LSAgent].allocationItems) {
            maxItemPrice = fmax(maxItemPrice, item->price);
        }
        double AgentEFMaxBundlePrice = agents[LSAgent].bundlePrice - maxItemPrice;
        alpha3 = fmin(alpha3, EFMaxBundlePrice/((AgentEFMaxBundlePrice<0)?0:AgentEFMaxBundlePrice));
    }
    return alpha3;
}
