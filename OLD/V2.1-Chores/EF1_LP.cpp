...
int main() {
    ...
    while(iteration < samples) {
        ...
        try {
            ...
            env.set("LogFile", "EF1_GurboiLog1.log");
            env.start();
            ...
            GRBVar Z[n];
            for(int i = 0; i < n; i++) {
                Z[i] = model.addVar(0.0, GRB_INFINITY, 0.0 , GRB_CONTINUOUS);   
            }
            ...
            for(int i = 0; i < n; i++) {
                for(int j = 0; j < n; j++) {
                    ...
                    model.addConstr(expr3<=Z[i]);
                    expr3.clear();
                }

                for(int k = 0; k < m; k++) {
                    model.addConstr( Z[i] >= (agents[i].itemUtilityMap[k] * X[i][k]) );
                }
            }
