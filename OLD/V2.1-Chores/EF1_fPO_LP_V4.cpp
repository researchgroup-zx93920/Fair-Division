...
// ------- Description V3: 
// ------- Y is contraint, Y constraints are directly added, objective is min (sum dX + sum dY)
// ------- we check EF1 and "fPO" property at the end 
...
int main() {
    ...
    while(iteration < samples) {
        ...
        try {
            ...
            for(int i = 0; i < n; i++) {
                for(int k = 0; k < m; k++) {
                    Y[i][k] = EF1_fPO_model.addVar(0.0, 1.0, 0.0 , GRB_CONTINUOUS);   
                }         
            }
            ...
            if (optimstatus == GRB_OPTIMAL) {
                ...
                for (int e = 0; e < nSolutions; e++) {
                    ...
                    for(int i = 0; i < n; i++) {
                        for(int k = 0; k < m; k++) {
                            y[i][k] = PO_model.addVar(0.0, 1.0, 0.0 , GRB_CONTINUOUS);   
                        }         
                    }
