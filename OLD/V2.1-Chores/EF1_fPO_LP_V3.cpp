...
// ------- Description V3: 
// ------- Y is binary, Y constraints are directly added, objective is min (sum dX + sum dY)
// ------- we check EF1 and "PO" property at the end 

// command to compile the programg++ -I/opt/gurobi952/linux64/include/ -L/opt/gurobi952/linux64/lib helper.o output.o -o EF1_LP EF1_LP.cpp -lgurobi_c++ -lgurobi95
...
int main() {
    ...
    while(iteration < samples) {
        ...
        try {
            ...
            EF1_fPO_model.set(GRB_DoubleParam_IntFeasTol, 1e-1); // set model tolerance
            ...
            if (optimstatus == GRB_OPTIMAL) {
                ...
                for (int e = 0; e < nSolutions; e++) {
                    ...
                    GRBEnv * env1 = 0;
                    env1 = new GRBEnv ();
                    GRBModel PO_model = GRBModel (*env1);
                    PO_model.set(GRB_IntParam_LogToConsole, 0);
                    PO_model.set(GRB_DoubleParam_FeasibilityTol, 1e-2);

                    // add the Y variable
                    vector<vector<GRBVar>> y(n, vector<GRBVar>(m));
                    for(int i = 0; i < n; i++) {
                        for(int k = 0; k < m; k++) {
                            y[i][k] = PO_model.addVar(0.0, 1.0, 0.0 , GRB_BINARY);   
                        }         
                    }
                    ...
                    for(int i = 0; i < n; i++) {
                        ...
                        PO_model.addConstr(expr1<=expr2);
                        expr1.clear();
                    }
                    ...
                    for(int i = 0; i < n; i++) {
                        for(int k = 0; k < m; k++) {
                            obj_PO+=(agents[i].itemUtilityMap[k]*y[i][k]);
                        }
                    }

                    PO_model.setObjective(obj_PO, GRB_MINIMIZE);
                    PO_model.optimize();

                    cout << "\n-------------------------------------------\n";
                    cout << "Printing X ():" <<  e+1 << "/" << nSolutions << "\n";
                    printGRBvar(X, n, m);
                    cout << "Printing Y ():" <<  e+1 << "/" << nSolutions << "\n";
                    printGRBvar(Y, n, m);

                    int optimstatus_PO = PO_model.get(GRB_IntAttr_Status);
                    if (optimstatus_PO == GRB_INF_OR_UNBD) {
                        cout << "Model is unbounded";
                        PO_model.set(GRB_IntParam_Presolve, 0);
                        PO_model.optimize();
                        optimstatus_PO = PO_model.get(GRB_IntAttr_Status);
                        return 1;
                    }
                    if (optimstatus_PO == GRB_OPTIMAL) {
                    ...
                    else if (optimstatus_PO == GRB_INFEASIBLE) {
...
void printGRBvar(vector<vector<GRBVar>> &X, int r, int c) {
    for(int i = 0; i < r; i++) {
        for(int k = 0; k < c; k++) {
            ...
            try{cout << abs(X[i][k].get(GRB_DoubleAttr_Xn)) << " ";}
            // catch(...) {cout << round(abs(X[i][k].get(GRB_DoubleAttr_X))) << " ";}  
            catch(...) {cout << abs(X[i][k].get(GRB_DoubleAttr_X)) << " ";}       
        }
        cout << "\n";
    }
    cout << "\n";
