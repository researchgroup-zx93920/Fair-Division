...
{
    // Define Inputs              
    int samples = 1300, iteration = 1200;    // number of samples to run the code for, iteration == seed
    ...
    while(iteration < samples) {
        ...
        while(1) {
            ...
            if(path_found) {
                ...
                prevLS = LS;
                prevBS = BS;
                // if the previous agent remains in the set of LSs and the element was not the , then search path from there instead
                if(prevLS>=0 && agents[prevLS].bundlePrice==minBundlePrice)
                    LS = prevLS;
                else
                    LS = leastSpenders[0];
                ...
                // if paths are not there from the first LS, only then reduce the set. If there will be paths from any
                // subsequent LSs then the LS set would already have been reset
                if(LS == leastSpenders[0]) {
            ...
            // LOGS
            DEBUG?(cout << "Searching alternating paths from LS: Agent " << LS << endl):(cout<< "");
            ...
            while(!q.empty()) {
                if(q.front()->type=="AgentNode") {
                    ...
                    if(visitedAgent[i]==0) {
                        ...
                        // if(doubleIsGreaterOrEqual(minBundlePrice, (agents[i].bundlePrice - currNode->price), EPS)==false ) {
                        if(doubleIsGreaterOrEqual(minBundlePrice, findEFMaxBundlePrice(agents, items, i), EPS)==false ) {
            ...
            if(pathViolater!=-1) {
                ...
                double beta = fmin(alpha1, alpha2); // alpha 2 in chores algorithm is not useful
                ...
                DEBUG?(cout << "----> Increasing Price of LS Component by beta = " << beta << endl):(cout<< "");
                DEBUG?(printRevisedPrices(items)):(printIntVector({}));
                DEBUG?(printAgentAllocationMBB(agents, items)):(printIntVector({}));
