...
void populateInstance(vector<AgentNodes> &agents, vector<ItemNodes> &items, double &minBundlePrice) {
    ...
    int m = items.size();
    for(int i = 0; i < n; i++) {
        double MBB = numeric_limits<double>::min();
        for(int j = 0; j < m; j++)
...
// //find EFMax Bundle Price of Big Spender
// double findEFMaxBundlePriceInComponent(vector<int> componentAgents, vector<AgentNodes> &agents, vector<ItemNodes> &items) {
//     double EFMaxBundlePrice = 0;
//     // else 
//     else {
//         for(int i = 0; i < agents.size(); i++) {
...
void transferItem(int itemToTransfer, int transferFromAgent, int transferToAgent, vector<AgentNodes> &agents, vector<ItemNodes> &items) {
    ...
    cout << std::setprecision(13) << "P(X_" << transferFromAgent << ")=" << agents[transferFromAgent].bundlePrice << endl;
    cout << std::setprecision(13) << "P(X_" << transferToAgent << ")=" << agents[transferToAgent].bundlePrice << endl;
    // agents[transferToAgent].bundlePrice << " " << items[itemToTransfer].price <<  " " << agents[transferToAgent].bundlePrice+items[itemToTransfer].price << endl;
...
bool is_EF1_fPO(vector<AgentNodes> &agents, vector<ItemNodes> &items) {
    ...
    for(int i = 0; i < agents.size(); i++) {
        for(int k = 0; k < agents.size(); k++) {
            ...
            if(findBundleValuation(i, i, agents)  < (findBundleValuation(k, i, agents) - maxValuation) && doubleIsEqual(findBundleValuation(i, i, agents), findBundleValuation(k, i, agents)  - maxValuation, EPS)==false ) {
                cout << "Failed for agent " <<  i << "-" << findBundleValuation(i, i, agents) << " and agent " << k  << "-" << findBundleValuation(k, i, agents)-maxValuation  << endl;
                return false;
            }
